...
function _makeTextData( hyouki : string, yomi : string ) : TextData[] {
...
//일단 save
function _compareTextData( hyoukiQuery : string, yomiQuery : string, textData : TextData[] ){
...
async function _getImiWithAI( videoId : string, db : db, revised : AutoDBData[] ){
    if( await _existApiKey() === false ){
        return revised;
    }

    if( fs.existsSync(`${assetPath}/transcript/${videoId}.wav_word_imi.json`) === true ){
        let _json = JSON.parse((await fs.readFileSync(`${assetPath}/transcript/${videoId}.wav_word_imi.json`)).toString());
...
async function getAutoDB(req : RouterRequest, res : RouterResponse){
...
async function postAutoDB(req : RouterRequest, res : RouterResponse){
    await db_connection(req, res, async(db) => {
        ...
        for(const [key, value] of Object.entries(change as Record<string, any>)){
...
async function getYomi(req : RouterRequest, res : RouterResponse){
...
async function getPrompt(req : RouterRequest, res : RouterResponse){
    await db_connection(req, res, async(db) => {
        ...
        const resultStr = (result as any[]).map( (v) => {
