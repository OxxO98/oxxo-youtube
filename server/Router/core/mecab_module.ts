...
import { AnalyzeTextObj, AnalyzeToken, ReadingToken, MecabToken, YomiToken } from "../../types/mecab_types.js"
...
function _kataToHira(str : string | null | undefined) : string {
...
export function _prefilter(tokens : AnalyzeToken[]) : AnalyzeToken[]{
    return tokens.filter(t =>
        t.surface.match(kanjiRegex) !== null
    );
}

function _complexKataToHira(origin : string, yomi : string | undefined) : string {
    if( yomi === undefined){
    ...
    if( _matched !== null ){
        ...
        if( _m_ori !== null && _m_yomi !== null ){
            let _hira = _m_yomi
                .filter( (v, i) => i !== 0).map( (v) => _kataToHira(v) );
            let _string = _m_ori
                .filter( (v, i) => i !== 0 )
                .reduce( (acc, curr, i) => acc.replace(curr, _hira[i]), origin );
...
export function _parse_buffer( text : string ) : Promise<string[][]> {
...
export async function getReadingWithMecab(text : string) : Promise<ReadingToken[]> {

    let _reg = /[\{\}\[\]\/?.,;:|\)`*`~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi
    let _text = text.replace(/\s+/g, '、').replace(_reg, '');

    let rows = await _parse_buffer( _text );
...
export async function checkWithMecab( text : string ) : Promise<MecabToken[]> {
...
export async function analyzeWithMeCab( textObj : AnalyzeTextObj ) : Promise<AnalyzeToken[]> {
...
export async function getYomiWithMecab( text : string ) : Promise<YomiToken[]> {
    let rows = await _parse_buffer( text );
