...
export const MakeDrftComp = ({ gotoTime } : MakeDraftCompProps ) => {
    ...
    const { timelineLoading } = useAppSelector( (state) => state.timeline );
    ...
    const handleTranscriptWithOption = () => {
        handleTranscript(videoId, value, { translate : isTranslate ? 'true' : 'false', prompt : isPrompt ? 'true' : 'false', lang : translationDirection === 'ja-ko' ? 'ja' : 'ko' })
    }
...
<>{
    ...
    const reHandleTranscript = () => {
        handleTranscript(videoId!, value, { reset : "true", translate : isTranslate ? 'true' : 'false', prompt : isPrompt ? 'true' : 'false', lang : translationDirection === 'ja-ko' ? 'ja' : 'ko' });
    }
