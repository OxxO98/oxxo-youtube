...
function useHandleSelection( document : Document, restrictId : string ) {
    ...
    const { selection, hurigana, selectedBun, textOffset, offset } = useAppSelector((_state) => _state.selection);
        ...
        if(_selection?.containsNode(_restrict, true)){
                ...
                if(_selection?.anchorNode?.parentNode?.nodeName === 'RUBY'){
                    if(_selection?.anchorNode?.nextSibling){
                        if(_selection?.anchorNode?.nextSibling?.nodeName === 'RT'){
                            dispatch( setHurigana( (_selection.anchorNode.nextSibling as HTMLElement).innerText ) );
                        }
                    }
                }
                else{
                    dispatch( setOffset({startOffset : _selection.anchorOffset, endOffset : _selection.focusOffset}) );
                    dispatch( setHurigana('') );
                }
            }
            ...
            if(_selection?.anchorNode?.parentNode && _selection?.focusNode?.parentNode){
                ...
                if(anchorTextNode?.getAttribute('data-bid') === focusTextNode?.getAttribute('data-bid')){
                    dispatch( setSelectedBun( anchorTextNode.getAttribute('data-bid') ?? '' ) );
                    ...
                    if(startOffset > endOffset){
                        dispatch( setTextOffset({startOffset : endOffset, endOffset : startOffset}) );
                    }
                    else{
                        dispatch( setTextOffset({startOffset : startOffset, endOffset : endOffset}) );
                    }
                }
            }
        }
